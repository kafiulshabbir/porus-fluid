#include "dat_radius.h"

namespace dat
{
	void cls_radius::error(const std::string& message) const
	{
		std::cout << name_file << " " << message << ", please run " << tool_generate_radius_distribution << ", and restart program.";
	}

	void cls_radius::multiply(matrix& w, double m)
	{
		for(auto& row: w)
		{
			for(auto& cell: row)
			{
				cell *= m;
			}
		}
	}

	bool cls_radius::outside_matrix(int m) const
	{
		return m < 0 || m >= n;
	}

	bool cls_radius::outside_matrix(const node& node) const
	{
		return outside_matrix(node.row) || outside_matrix(node.col);
	}

	int cls_radius::good_begin(int a, int b) const
	{
		return std::min(a, b);
	}

	double cls_radius::extract_radius(const node& node_i, const node& node_j) const
	{
		if(node_i.vertical(node_j))
		{
			if(outside_matrix(node_j))
			{
				return 0;
			}
			
			int begin = good_begin(node_i.row, node_j.row);
			return ver[begin][node_i.col];
		}
		if(node_i.horizontal(node_j))
		{
			if(outside_matrix(node_j))
			{
				return leak_radius;
			}
			
			int begin = good_begin(node_i.col, node_j.col);
			return hor[node_i.row][begin];
		}
		
		std::cout << "porgram failed due to non-adjacent radius access attempt!";
		throw std::invalid_argument("the two nodes are not adjacent to each other, and a tube between them does not exist!");
		return -1;
	}

	cls_radius::cls_radius(const std::string& name_file): name_file(name_file) {}

	bool cls_radius::read_file()
	{
		std::ifstream fin(name_file);
		if(!fin.is_open())
		{
			error("does not exist");
			return true;
		}
		
		std::string s;
		fin >> s >> n;
		
		if(s != radius_distribution_txt_first_line)
		{
			error("has incorrect format");
			return true;
		}
		
		double multiplier = ReadNamedData(fin);
		leak_radius = multiplier * ReadNamedData(fin);
		
		
		ver = ReadMatrix(n - 1, n, fin);
		hor = ReadMatrix(n - 1, n, fin);
		
		multiply(ver, multiplier);
		multiply(hor, multiplier);
		
		std::cout << name_file << " correctly read!\n";
		return false;
	}

	double cls_radius::operator () (const node& node_i, const node& node_j) const
	{
		return extract_radius(node_i, node_j);
	}
	
	double cls_radius::operator()(const node& node_i, const node& node_j, int power) const
	{
		return std::pow(extract_radius(node_i, node_j), power);
	}

	int cls_radius::N() const
	{
		return n;
	}
}
