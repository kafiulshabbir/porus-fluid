...
void Draw(matrix radius, matrix fill, const int count, const float time_ran)
{
...
matrix PressureRelationMatrix(const matrix& radius, const matrix& fill)
{
...
matrix Velocity(const std::vector<std::vector<float>>& radius, const std::vector<std::vector<float>>& fill, const std::vector<float>& pressure)
{
...
matrix Spread(matrix fill)
{
	...
	for(int i = 0; i < n; ++ i)
	{
		for(int j = 0; j < m; ++ j)
		{
			auto& f = std::abs(fill[i][j]);
			if(f > 1.0 - THRESHOLD_FILL)
			{
				f = 1;
			}
...
int main()
{
	...
	float time_ran = 0;
	int count = 0;
	Draw(radius, fill, count, time_ran);
	while(time_ran < TIME_SIMULATE)
	{
		...
		auto pressure_relation_matrix = PressureRelationMatrix(radius, fill);
		auto pressure_vector = GaussElimination(pressure_relation_matrix);
		auto velocity = Velocity(radius, fill, pressure_vector);
