...
const std::string FILE_IN_NAME_FILL_DISTRIBUTION = "in_fill_distribution";
const std::string FILE_IN_NAME_RADIUS_DISTRIBUTION = "in_radius_distribution";
...
RadiusDistribution ReadRadiusDistribution()
{
	...
	ver = ReadMatrix(file.n - 1, file.n, fin);
	hor = ReadMartix(file.n, file.n - 1, fin);
	
	return file;
}

FillDistribution ReadFillDistribution(
struct colour
{
	int r;
	int g;
...
class bmp
{
	...
	static unsigned int pad(int a, int b)
	{
		return ((a + b - 1) / b ) * b - a;
	}
	
	
	int determine_padding()
	{
...
int main()
{
	...
	bmp a(4000, 3000, white);
	a.drawRectangle(0, 0, 1000, 2000);
