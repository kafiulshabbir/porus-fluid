 ...
 */
static float mns::_scontb_sig(bool condition) //scontb's sign function
{
...
static mns::Ccmprt mns::merge_existing_and_new_cmprts(mns::Ccmprt& cmprt_existing, const mns::Ccmprt& cmprt_new, float vel)
{
	if(vel > 0)
	{
...
static std::vector<float> mns::gen_pos_from_segmented(std::vector<float> pos_segmented)
{
...
{
	const float area = PI * std::pow(r, 2);
